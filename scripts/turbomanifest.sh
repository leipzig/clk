...
#mkdir GRCh38_star
aws s3 cp s3://clk-splicing/refs/GRCh38_star/${gtf} .
...
echo "running rmats.."
mkdir -p ${comparison}
python2.7 /rMATS.4.0.2/rMATS-turbo-Linux-UCS4/rmats.py --nthread ${cpus} -t paired --gtf ${gtf} --b1 <( python2.7 /rMATS.4.0.2/rMATS-turbo-Linux-UCS4/manifest_to_csl.py manifest.list 1 . ) --b2 <( python2.7 /rMATS.4.0.2/rMATS-turbo-Linux-UCS4/manifest_to_csl.py manifest.list 2 . ) --od ${comparison} >${comparison}/rmats.out.txt 2>&1
